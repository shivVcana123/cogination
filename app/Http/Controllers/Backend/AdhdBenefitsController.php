...
class AdhdBenefitsController extends Controller
{
    ...
    public function saveAdhdBenefits(Request $request)
    {
        ...
        $existingPointers = $adhdBenefit->pointers ? json_decode($adhdBenefit->pointers, true) : [];
    ...
    public function adhdFirstSection(){
        $adhdfirstSection = AdhdChildSection::where('type','=','Child')->get();
        return view('adhd-section.adhdfirstsection',compact('adhdfirstSection'));
    }
    public function fetchAdhdSectionByType(Request $request)
{
    ...
    $adhdSection = AdhdChildSection::where('type', $type)->get();

    return response()->json(['data' => $adhdSection]);
}

    public function saveAdhdFirstSection(Request $request)
    {
        dd($request->all());
        // Validate the request data
        $validated = $request->validate([
        ...
        $adhdfirstSection = $request->id 
            ? AdhdChildSection::find($request->id) 
            : new AdhdChildSection();
        ...
        return redirect()->route('adhd-first-section')->with('success', 'Adhd details saved successfully.');
    }

    public function adhdSecondSection(){
        $adhdsecondsection = AdhdChildSection::where('type','=','Adult ')->get();
        return view('adhd-section.adhdsecondsection',compact('adhdsecondsection'));
    }
    public function saveAdhdSecondSection(Request $request)
    {
        // Validate the request data
        $validated = $request->validate([
            'first_title' => 'required|string|max:255',
