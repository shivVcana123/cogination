                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        <div class="card-header" style="background-color:#0476b4">
                            <h3 class="card-title">{{ empty($adhdfirstSection) || !isset($adhdfirstSection[0]) ? 'Add' : 'Edit' }} First Section Details</h3>
                        </div>
                        <form action="{{ route('save-first-section') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" value="{{ old('id', $adhdfirstSection[0]->id ?? '') }}">
                            <div class="card-body">
                            <div class="form-group">
                                    ...
                                    <option disabled selected>Please Select Type</option>
                                        <option value="Child" {{ old('type',($adhdfirstSection[0]->type === 'Child') ? 'selected' : '' ?? '') }}>Child</option>
                                        <option value="Adult" {{ old('type',($adhdfirstSection[0]->type === 'Adult') ? 'selected' : '' ?? '') }}>Adult</option>
                                    </select>
                                    @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="first_title" id="title"
                                        placeholder="Enter first title" value="{{ old('first_title',$adhdfirstSection[0]->first_title ?? '') }}">
                                    @error('first_title')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="first_subtitle" id="subtitle"
                                        placeholder="Enter first subtitle" value="{{ old('first_subtitle',$adhdfirstSection[0]->first_subtitle ?? '') }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="first_description" id="description">{{ old('first_description', $adhdfirstSection[0]->first_description ?? '') }}</textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="first_button_content" id="button_content" placeholder="Enter Button Text" value="{{old('first_button_content',$adhdfirstSection[0]->first_button_content ?? '')}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="first_button_link" id="button_link" placeholder="Enter Button Link" value="{{old('first_button_link',$adhdfirstSection[0]->first_button_link ?? '')}}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <img id="blah" src="{{asset($adhdfirstSection[0]->first_image ?? '')}}" alt="Image Preview" style="width: 130px; display:none" />
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="second_title" id="second_title"
                                        placeholder="Enter second title" value="{{ old('second_title',$adhdfirstSection[0]->second_title ?? '') }}">
                                    @error('second_title')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="second_subtitle" id="second_subtitle"
                                        placeholder="Enter second subtitle" value="{{ old('second_subtitle',$adhdfirstSection[0]->second_subtitle ?? '') }}">
                                ...
                                <div class="form-group">
                                    <label for="description_1">Description</label>
                                    <i class="fas fa-info-circle" title="Describe the purpose or details of this section in 2-3 sentences."></i>
                                    <textarea class="form-control" name="second_description" id="second_description">{{ old('second_description', $adhdfirstSection[0]->second_description ?? '') }}</textarea>
                                    @error('second_description')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                @php
                                // Check if the $adhdfirstSection exists and contains data before attempting to decode
                                $pointers = isset($adhdfirstSection[0]) && !empty($adhdfirstSection[0]->pointers)
                                ? json_decode($adhdfirstSection[0]->pointers)
                                : [];
                                @endphp
                                ...
                                <button type="button" class="btn btn-success" id="add-Pointers">Add Pointer</button>
                                    ...
                                    <div class="form-group">
                                        <label for="image">Image</label>
                                        <i class="fas fa-info-circle" title="Upload an image that visually represents this section."></i>
                                        <img id="second_img" src="{{asset($adhdfirstSection[0]->second_image ?? '')}}" alt="Image Preview" style="width: 130px; display:none" />
...
//         if (selectedType) {
//             $.ajax({
//                 url: "{{ route('fetch-adhd-section-by-type') }}", // Ensure this route exists in your Laravel app
//                 type: "GET",
...
//                                 $('#blah').attr('src', `{{ asset('') }}${section.first_image}`).show();
//                             } else {
//                                 $('#blah').hide();
//                             }
//                             if (section.second_image) {
//                                 $('#second_img').attr('src', `{{ asset('') }}${section.second_image}`).show();
//                             } else {
//                                 $('#second_img').hide();
//                             }
