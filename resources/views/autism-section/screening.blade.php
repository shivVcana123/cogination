...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Autism Screening Section</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        <div class="card-header" style="background-color:#0476b4">
                            <h3 class="card-title">{{ empty($autismScreening) ? 'Add' : 'Edit' }} Screening Section</h3>
                        </div>
                        <form action="{{ route('save-screening-section') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" id="id" value="{{ old('id', $autismScreening[0]->id ?? '') }}">
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select name="type" id="type" class="form-control">
                                        ...
                                        <option value="Child" {{ old('type', $autismScreening[0]->type ?? '') === 'Child' ? 'selected' : '' }}>Child</option>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" class="form-control" name="title" id="title" placeholder="Enter title" value="{{ old('title', $autismScreening[0]->title ?? '') }}" required>
                                        @error('title')
                                        <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description" class="form-control" required>{{ old('description', $autismScreening[0]->description ?? '') }}</textarea>
                                    @error('description')
                                        <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                </div>
...
<script>
    ...
    $('#type').on('change', function() {
        ...
        if (selectedType) {
            $.ajax({
                url: "{{ route('fetch-screening-section-by-type') }}",
                type: "GET",
                data: {
                    type: selectedType
                },
                    ...
                    if (response && response.data && response.data.length > 0) {
                        ...
                        if (section) {
                            ...
                            const imageUrl = section.image || '';
                                $('#img').attr('src', imageUrl ? imageUrl : '#'); 
                        }
                    } else {
